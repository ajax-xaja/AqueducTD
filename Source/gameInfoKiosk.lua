...
--[[	
		...
		for i=1, #tileArray do
         ...
         levelRects[i].x = xPos; levelRects[i].y = yPos
			...
			if placementAllowed == true then 
				...
				levelRects[i]:addEventListener("tap", levelTouched)
			end
		end
		
		------------------------
		-- Sets up Kiosk Towers
		-------------------------
		local function kioskTowers( )
			local tower = Towers.BoomerangTower:new(144, 144, enemyGroup)
			table.insert(towerArray, 44, tower)
			local tower = Towers.PistolTower:new(272, 208, enemyGroup)
			table.insert(towerArray, 68, tower)
			local tower = Towers.MagnumTower:new(208, 240, enemyGroup)
			table.insert(towerArray, 76, tower)
			local tower = Towers.BallChainTower:new(272, 144, enemyGroup)
			table.insert(towerArray, 48, tower)
		end
	...
	-- KIOSK TEXT Listener
	---------------------
		local function kioskTextListener( event )
			audio.stop(Sounds.bgChannel)
			gameIsActive = false
			timer.cancel(attackSpeedTimerUpdater)
	...
	-- KIOSK TEXT 
	---------------------
		kioskText = display.newText("Click anywhere to play!", 0, 0, "Helvetica", 30)
		kioskText.x = _W*0.5; kioskText.y = _H*0.35; overlayGroup:insert(kioskText)
		kioskText:addEventListener( "tap", kioskTextListener )
	end
	...
	local function levelComplete()
		...
		storyboard.gotoScene( "gameInfoKiosk"..currentLevel, completeOptions )
